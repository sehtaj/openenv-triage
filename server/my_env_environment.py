...
class MyEnvironment(Environment[TriageAction, TriageObservation, TriageState]):
    ...
    def step(
        ...
        **_: object,
        ...
        reward = case.action_value(requested_decision)
        review_honored = requested_decision == "review"

        if requested_decision == "review":
            if self._episode.remaining_review_budget > 0:
                self._episode.remaining_review_budget -= 1
                self._episode.reviewed_count += 1
            else:
                reward = REVIEW_EXHAUSTION_PENALTY - case.review_cost
